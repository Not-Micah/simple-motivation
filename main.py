...
class Main(MDApp):
    ...
    @run_on_ui_thread
    def statusbar(self, top, bottom):
        window = activity.getWindow()
        window.clearFlags(WindowManager.FLAG_TRANSLUCENT_STATUS)
        window.addFlags(WindowManager.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)
        window.setStatusBarColor(Color.parseColor(top)) 
        window.setNavigationBarColor(Color.parseColor(bottom))
    ...
    def load_theme(self):
        if self.theme == "dark":
            ...
            self.statusbar(top="#121212", bottom="#191919")
            ...
            self.statusbar(top="#F6F6F6", bottom="#DDDDDD")
